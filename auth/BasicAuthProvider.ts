...
export class BasicAuthProvider implements core.AuthProvider {
    ...
    public async getAuthRequest({
        ...
        if (username == null) {
            throw new errors.VobizError({ message: BasicAuthProvider.AUTH_CONFIG_ERROR_MESSAGE_USERNAME });
        }
        const password = await core.Supplier.get(this.options[PASSWORD_PARAM]);
        if (password == null) {
            throw new errors.VobizError({ message: BasicAuthProvider.AUTH_CONFIG_ERROR_MESSAGE_PASSWORD });
        }
